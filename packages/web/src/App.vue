...
import { refreshFragments, fragmentsState } from './stores/fragments';
...
onMounted(async () => {
  try {
    // 先载入片段库，再载入文档：文档中的 $fragment 在首次解析时即可被解释
...
<template>
  <div class="app-shell">
    ...
    <div v-if="fragmentsState.message" class="flash">{{ fragmentsState.message }}</div>
  </div>
</template>
